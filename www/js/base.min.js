...
var subpage = {
  resize: function(){
    $("#subpage .subpage-resize").css({"height":$(window).height()});
    $("#subpage .subpage-ctrol").css({"height":$(window).height()});
  },
  show: function(){
    var suboffset = $(".subpage-resize").width();
    var status = $("#subpage-button").data("status");
    if(status == "show"){
      $("#addfeed-panel").css("position","fixed");
      $("#subpage").animate({"left":"0px"},function(){
        $("#subpage-button").data("status","hide").html('<i class="icon-chevron-left"></i>');
      });
    } else if(status == "hide"){
      $("#addfeed-panel").css("position","absolute");
      $("#subpage").animate({"left":-suboffset},function(){
        $("#subpage-button").data("status","show").html('<i class="icon-chevron-right"></i>');
      });
    }
  }
};
